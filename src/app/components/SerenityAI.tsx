...
import { Sparkles, TrendingUp, Moon, Utensils, Zap } from 'lucide-react';
import { motion } from 'framer-motion';
import { FeedLog, SleepLog } from '../../types';

const MotionDiv = motion.div as any;

interface SerenityAIProps {
  feeds: FeedLog[];
  sleeps: SleepLog[];
}

export const SerenityAI: React.FC<SerenityAIProps> = ({ feeds, sleeps }) => {
  const insight = useMemo(() => {
    ...
    const avgInterval = 3; // Default 3 hours
    
    const feedType = feeds.filter(f => f.type === 'breast').length > feeds.length / 2 ? 'breastfeeding' : 'bottle feeding';
    ...
    >
       ...
       <div className="flex items-center gap-4 mb-2">
          <div className={`${insight.color} flex items-center gap-2`}>
             <Sparkles size={16} />
             <span className="text-[10px] font-black uppercase tracking-[0.2em]">Serenity AI Insight</span>
          </div>
       </div>
       ...
       <p className="text-xs font-medium text-text-dim leading-relaxed max-w-[240px]">
          {insight.desc}
       </p>
    </MotionDiv>
  );
