...
import { getJournalEntriesByBaby, getMemoryLogsByBaby } from '../../lib/supabase-storage';
import { JournalEntry, MemoryLog } from '../../types';
...
export const AIScrapbook: React.FC<AIScrapbookProps> = ({ onBack }) => {
  ...
  const [scrapbookContent, setScrapbookContent] = useState<{
    title: string;
    summary: string;
    highlights: string[];
    vibe: string;
  ...
  const generateScrapbook = async () => {
    ...
    try {
      // Fetch latest logs to "analyze"
      const [entries, memories] = await Promise.all([
        getJournalEntriesByBaby(currentBaby.id),
      ...
      // Mocked AI output based on real data found
      const month = new Date().toLocaleString('default', { month: 'long' });
      const year = new Date().getFullYear();

      setScrapbookContent({
        title: `${currentBaby.name}'s ${month} Magic`,
        summary: `This month was defined by ${memories.length > 0 ? memories.length : 'countless'} moments of growth. From ${entries[0]?.text.slice(0, 30) || 'quiet cuddles'} to the way ${currentBaby.name} is starting to explore the world, your home is filled with progress.`,
        highlights: [
          entries[0]?.text.slice(0, 60) || "The way eyes light up when seeing a familiar face.",
          memories[0]?.text.slice(0, 50) || "The distinct personality blooming with every smile.",
          "A newfound curiosity for textures and sounds.",
          "Developing a unique 'laugh' that melts your heart."
        ].filter(Boolean),
        vibe: "Warm, nurturing, and full of discovery."
      });
    } catch (err) {
             ...
             <div className="py-20 text-center space-y-8">
                ...
                <div className="space-y-3">
                   ...
                   <p className="text-sm font-bold text-orange-900/60 dark:text-orange-100/40 leading-relaxed px-4">
                      Let Serenity AI transform your daily journal entries and memories into a beautiful digital scrapbook page.
                   </p>
                </div>
                <button 
              ...
              >
                <div className="p-10 space-y-10">
                   <div className="text-center space-y-4">
                      <div className="w-12 h-1 h-1 bg-orange-200 dark:bg-orange-900 mx-auto rounded-full" />
                   ...
                   <div className="space-y-4">
                      ...
                      <div className="space-y-4">
                         {scrapbookContent.highlights.map((h, i) => (
                           <div key={i} className="flex gap-4 p-5 bg-[#FAF9F6] dark:bg-zinc-800/50 rounded-[2.5rem] border border-orange-50 dark:border-zinc-800">
                   ...
                   <div className="pt-6 flex items-center justify-between border-t border-orange-50 dark:border-zinc-800">
                      ...
                      <div className="flex gap-3">
                         <button className="w-10 h-10 rounded-full bg-orange-100 dark:bg-orange-900/30 text-orange-600 dark:text-orange-400 flex items-center justify-center hover:scale-110 transition-all"><Download size={18} /></button>
                         <button className="w-10 h-10 rounded-full bg-orange-500 text-white flex items-center justify-center shadow-lg shadow-orange-500/20 hover:scale-110 transition-all"><Share2 size={18} /></button>
                      </div>
                   </div>
                </div>
                
                {/* Visual Polaroid Stickers */}
